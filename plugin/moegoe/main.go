...
const (
	...
	/*jpapi = "127.0.0.1:8000/%s?text=%s"
	krapi = "127.0.0.1:8000/%s?text=%s"*/
)
...
func init() {
	...
	/*en.OnRegex(`^让(.*)说(.*)$`).Limit(ctxext.LimitByGroup).SetBlock(true).
	Handle(func(ctx *zero.Ctx) {
		text := ctx.State["regex_matched"].([]string)[2]
		name := ctx.State["regex_matched"].([]string)[1]
		ctx.SendChain(message.Record(fmt.Sprintf(jpapi, name, text)))
	})*/
}
